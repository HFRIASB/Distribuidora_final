...
function listar_venta(){
    ...
    t_venta = $("#tabla_ventas").DataTable({
      ...
      "columns":[
            ...
            {"defaultContent":"<button class='imprimir btn btn-primary'><i class='fa fa-print'></i></button>&nbsp;<button class='anular btn btn-danger'><i class='fa fa-trash'></i></button>"}
...
$('#tabla_ventas').on('click','.imprimir',function(){
    ...
    window.open("../MPDF/generar_venta.php?codigo="+parseInt(data.venta_id)+"#zoom=100","Reporte Ingreso","scrollbards=NO");
})
...
function Agregar_Producto_Detalle_Venta(){
    ...
    if(parseFloat(stock) < parseFloat(cantidad)){
        return Swal.fire("Mesaje de Advertencia","El producto no tiene el stock suficiente","warning");
    }
...
function SumarTotalneto() {
    ...
    let tipo = document.getElementById('cbm_tipo').value;
    if(tipo=="FACTURA"){
        $("#lbl_subtotal").html("<b>Sub Total: </b> Bs."+subtotal);
        //$("#lbl_impuesto").html("<b>IVA "+impuesto*100+"%: </b> Bs."+impuestototal);
        $("#lbl_impuesto").html("Bs."+impuestototal);
        $("#lbl_totalneto").html("<b>Total: </b> Bs."+total.toFixed(2));
    }else{
        $("#lbl_totalneto").html("<b>Total: </b> Bs."+total.toFixed(2));
    }
}
...
function Registar_Venta(){
    ...
    if(tipo=="FACTURA"){
         ...
         impuesto = document.getElementById('lbl_impuesto').innerHTML.substr(3);;
    }else{
        porcentaje = 0;
        impuesto = 0;
    }
    ...
    $.ajax({
        ...
        if(resp>0){
            Swal.fire({
                ...
                if (result.value) {
                    window.open("../MPDF/generar_venta.php?codigo="+parseInt(id)+"#zoom=100","Reporte Ingreso","scrollbards=NO");
